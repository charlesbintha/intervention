...
@section('title', 'Modifier Survey - Portail Intervention')

@section('content')
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-8">
        <a href="{{ route('surveys.show', $survey) }}" class="text-gut-blue hover:text-opacity-75">
            <i class="fas fa-arrow-left mr-2"></i>Retour au survey
        </a>
        <h1 class="text-3xl font-bold text-gray-800 mt-4">Modifier le Survey</h1>
        <p class="text-gray-600 mt-2">{{ $survey->opportunity_name }}</p>
    </div>

    <form action="{{ route('surveys.update', $survey) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-lg p-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <select name="subsidiary" id="subsidiary"
                    ...
                    <option value="GUT" {{ old('subsidiary', $survey->subsidiary) === 'GUT' ? 'selected' : '' }}>Groupe Univers Telecom (GUT)</option>
                    <option value="CP" {{ old('subsidiary', $survey->subsidiary) === 'CP' ? 'selected' : '' }}>Cabinet Pencco (CP)</option>
                    <option value="UTA" {{ old('subsidiary', $survey->subsidiary) === 'UTA' ? 'selected' : '' }}>Univers Telecom Afrique (UTA)</option>
                    <option value="UA" {{ old('subsidiary', $survey->subsidiary) === 'UA' ? 'selected' : '' }}>Univers Academy (UA)</option>
                    <option value="UTE" {{ old('subsidiary', $survey->subsidiary) === 'UTE' ? 'selected' : '' }}>Univers Technology & Energy (UTE)</option>
                    <option value="UC" {{ old('subsidiary', $survey->subsidiary) === 'UC' ? 'selected' : '' }}>Univers Capital (UC)</option>
                </select>
                @error('subsidiary')
            ...
            <div>
                <label for="opportunity_name" class="block text-sm font-medium text-gray-700 mb-2">Nom de l'opportunité *</label>
                <input type="text" name="opportunity_name" id="opportunity_name" value="{{ old('opportunity_name', $survey->opportunity_name) }}" required
            ...
            <div>
                ...
                <input type="text" name="company_name" id="company_name" value="{{ old('company_name', $survey->company_name) }}" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gut-blue focus:border-transparent">
            </div>

            <div class="md:col-span-2">
                <label for="location" class="block text-sm font-medium text-gray-700 mb-2">Lieu *</label>
                <input type="text" name="location" id="location" value="{{ old('location', $survey->location) }}" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gut-blue focus:border-transparent">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="text" name="contact_name" id="contact_name" value="{{ old('contact_name', $survey->contact_name) }}" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gut-blue focus:border-transparent">
            </div>

            <div>
                <label for="contact_function" class="block text-sm font-medium text-gray-700 mb-2">Fonction *</label>
                <input type="text" name="contact_function" id="contact_function" value="{{ old('contact_function', $survey->contact_function) }}" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gut-blue focus:border-transparent">
            </div>

            <div>
                <label for="contact_phone" class="block text-sm font-medium text-gray-700 mb-2">Téléphone *</label>
                <input type="tel" name="contact_phone" id="contact_phone" value="{{ old('contact_phone', $survey->contact_phone) }}" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gut-blue focus:border-transparent">
            </div>

            <div>
                <label for="contact_email" class="block text-sm font-medium text-gray-700 mb-2">Courriel *</label>
                <input type="email" name="contact_email" id="contact_email" value="{{ old('contact_email', $survey->contact_email) }}" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gut-blue focus:border-transparent">
            </div>
        </div>

        <div class="border-l-4 border-gut-blue pl-4 mb-8">
            <h2 class="text-xl font-semibold text-gray-800 mb-4">Détails du Survey</h2>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                <label for="start_datetime" class="block text-sm font-medium text-gray-700 mb-2">Date et heure de début *</label>
                <input type="datetime-local" name="start_datetime" id="start_datetime" value="{{ old('start_datetime', $survey->start_datetime->format('Y-m-d\TH:i')) }}" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gut-blue focus:border-transparent">
            </div>

            <div>
                <label for="end_datetime" class="block text-sm font-medium text-gray-700 mb-2">Date et heure de fin *</label>
                <input type="datetime-local" name="end_datetime" id="end_datetime" value="{{ old('end_datetime', $survey->end_datetime->format('Y-m-d\TH:i')) }}" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gut-blue focus:border-transparent">
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="purpose" id="purpose" rows="4" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gut-blue focus:border-transparent">{{ old('purpose', $survey->purpose) }}</textarea>
            </div>
        </div>
        ...
        <div class="mb-6">
            ...
            <div id="intervenants-gut-container" class="space-y-4">
                @foreach($survey->intervenantsGut as $index => $intervenant)
        ...
        <div class="mb-6">
            ...
            <div id="intervenants-rencontres-container" class="space-y-4">
                @foreach($survey->intervenantsRencontres as $index => $intervenant)
                    <div class="border border-gray-200 rounded-lg p-4 bg-gray-50">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="flex justify-end space-x-4 pt-6 border-t">
            <a href="{{ route('surveys.show', $survey) }}" class="px-6 py-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition">
                Annuler
            </a>
...
@push('scripts')
<script>
let gutCounter = {{ $survey->intervenantsGut->count() }};
let rencontreCounter = {{ $survey->intervenantsRencontres->count() }};
